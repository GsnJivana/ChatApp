                    ...
                    @if(isset($selectedConversation))
                        <div class="card">
                            <div class="card-header">
                                Conversation avec
                                {{ $selectedConversation->users->where('id', '!=', auth()->id())->first()->name }}
                            </div>
                            <div class="card-body" id="chat-box" style="height: 400px; overflow-y: scroll;">
                                ...
                                @foreach($selectedConversation->messages as $message)
                                    <div class="mb-2 {{ $message->user_id == auth()->id() ? 'text-end' : '' }}">
                                        ...
                                        <span
                                            class="badge rounded-pill {{ $message->user_id == auth()->id() ? 'bg-primary' : 'bg-secondary' }}">
                                            {{ $message->body }}
                                        </span>
                                    </div>
                                @endforeach
                            ...
                            <div class="card-footer">
                                <form id="message-form" action="{{ route('chat.store', $selectedConversation->id) }}"
                                    method="POST">
                                    @csrf
